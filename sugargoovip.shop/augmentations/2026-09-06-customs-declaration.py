...
DATA_PATH = (
    ...
    / "article-data"
    / "2026-09-06-customs-declaration.json"
)
...
def render_article():
    template = (ROOT / "guide-sugargoo-tracking-not-updating.html").read_text(
        encoding="utf-8"
    )
    toc = "".join(
    ...
    hero = f'''<section class="article-hero"><div class="frame article-hero-grid"><div><span class="article-label">Customs Declaration &amp; Import Proof · Fact checked</span><h1>{esc(TITLE)}</h1><p class="article-deck">{esc(DECK)}</p><div class="article-meta"><span>Published {DISPLAY}</span><span>12 min read</span><span>{WORD_COUNT:,} words</span></div></div><div class="article-hero-card"><small>Independent buyer guide</small><strong>14</strong><p>Original English editorial content based on current Sugargoo official guidance. Customs rules, tax thresholds, route terms and documentation requirements must be rechecked for the destination before shipping.</p></div></div></section>'''
    template = re.sub(
    ...
    related = '''<div class="article-bottom-nav"><a href="guide-shipping-cost.html">Shipping cost guide</a><a href="guide-sugargoo-packing-center-parcel.html">Packing Center guide</a><a href="guides.html">All buyer guides →</a></div>'''
    shell = f'''<div class="frame article-shell"><aside class="article-toc"><span>On this page</span><nav>{toc}</nav></aside><article class="article-main"><p class="article-intro">A defensible customs declaration starts with an accurate parcel inventory, not with a number copied from somebody else's shipment.</p><div class="article-factbox"><b>Research standard</b><p>Facts were checked against Sugargoo's official customs, route-selection, tax-handling and insurance guidance on {DISPLAY}. The declaration-led workflow below was written from scratch for SugargooVIP.</p></div>{body}<section class="article-faq" id="faq"><h2>FAQ: Sugargoo customs declarations and clearance</h2>{faq_html}</section><div class="article-tags">{''.join(f'<span>{esc(tag)}</span>' for tag in TAGS)}</div>{related}</article></div>'''
    template = re.sub(
    ...
    if TITLE not in template or template.count("<details>") != 10:
...
def patch_guides():
    ...
    page = re.sub(
        r"<title>.*?</title>",
        "<title>Sugargoo Buyer Guides 2026: Customs, Tracking, Insurance, Warehouse, QC and Shipping</title>",
        page,
        count=1,
        flags=re.S,
    )
    page = re.sub(
        r'<meta content="[^"]*" name="description"/>',
        '<meta content="Read fourteen original Sugargoo buyer guides covering customs declarations, tracking, insurance, DIY orders, warehouse decisions, shipping, QC and W2C." name="description"/>',
        page,
        count=1,
    )
    page = re.sub(
    ...
    page, count = re.subn(
        r"<p>(?:Thirteen|Fourteen) long-form reverse-shopping guides.*?</p>",
        "<p>Fourteen long-form reverse-shopping guides written from scratch after checking Sugargoo's current official documentation. Every historical article remains available, newest first.</p>",
        page,
        count=1,
        flags=re.S,
    )
    if count != 1:
        raise RuntimeError("Guide count introduction not found")
    if SLUG not in page.split('<div class="guide-directory">', 1)[1]:
        card = f'''<article><a class="guide-directory-cover shipping" href="{SLUG}"><span>Customs declaration &amp; proof</span><b>01</b></a><div class="guide-directory-body"><h2><a href="{SLUG}">{esc(TITLE)}</a></h2><p>{esc(META)}</p><div class="guide-directory-meta"><span>{DISPLAY}</span><span>12 min read</span></div><a href="{SLUG}">Read the full {WORD_COUNT:,}-word guide →</a></div></article>'''
        page = page.replace(
...
def patch_home():
    ...
    section = section[:grid_start] + new_card + "".join(cards) + section[grid_end:]
    section = re.sub(
        r'<div class="latest-guides-intro"><p>.*?</p>',
        '<div class="latest-guides-intro"><p>Three practical Sugargoo guides covering customs declarations, parcel tracking and shipment insurance. Newest articles appear first.</p>',
        section,
        count=1,
        flags=re.S,
    )
...
def patch_sitemap():
    ...
    if SLUG not in xml:
        marker = (
            ...
            "guide-sugargoo-tracking-not-updating.html"
        )
        entry = (
...
checks = {
    ...
    "guides_cards": guides.count("<article>") == 14,
    "guides_schema_count": '"numberOfItems":14' in guides,
    "sitemap": SLUG in sitemap,
}
if WORD_COUNT != 1789 or checks["faq_count"] != 10:
    raise RuntimeError(checks)
if not all(value for key, value in checks.items() if key not in {"word_count", "faq_count"}):
    raise RuntimeError(checks)
