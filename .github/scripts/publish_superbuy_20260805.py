...
slug = "superbuy-first-parcel-workflow-2026"
route = f"/guides/{slug}/"
...
for required in [slug, "superbuy-restricted-items-parcel-risk-checklist-2026", "superbuy-warehouse-storage-strategy-2026"]:
    assert required in latest_segment, f"missing homepage guide: {required}"
positions = [latest_segment.index(x) for x in [slug, "superbuy-restricted-items-parcel-risk-checklist-2026", "superbuy-warehouse-storage-strategy-2026"]]
...
archive_card = '''<a class="guide" href="/guides/superbuy-first-parcel-workflow-2026/"><div class="guide-cover"><small>AUGUST 5, 2026</small><strong>Superbuy First Parcel Workflow 2026</strong></div><div class="guide-body"><p>Six checkpoints for fees, restrictions, QC, coupons, packaging and freight reconciliation.</p><div class="read"><span>1,664 words</span><span>Read →</span></div></div></a>'''
...
guides = guides.replace(
    'restricted items, warehouse storage, shipping costs, QC photos, W2C research, fees, coupons and country-specific parcel planning.',
    'first-parcel planning, restricted items, warehouse storage, shipping costs, QC photos, W2C research, fees, coupons and country-specific parcel planning.',
    1,
)
assert guides.count(route) == 1, "new guide must appear once in archive"
for preserved in [
    "superbuy-restricted-items-parcel-risk-checklist-2026",
    "superbuy-warehouse-storage-strategy-2026",
    "superbuy-shipping-costs-2026",
    "superbuy-qc-photos",
    "superbuy-w2c",
]:
...
sitemap = re.sub(r'(<loc>https://superbuyvip\.shop/</loc><lastmod>)\d{4}-\d{2}-\d{2}', r'\g<1>2026-08-05', sitemap, count=1)
sitemap = re.sub(r'(<loc>https://superbuyvip\.shop/guides/</loc><lastmod>)\d{4}-\d{2}-\d{2}', r'\g<1>2026-08-05', sitemap, count=1)
entry = f'<url><loc>https://superbuyvip.shop{route}</loc><lastmod>2026-08-05</lastmod><priority>0.9</priority></url>\n'
...
# Validate technical SEO, mobile rules, word count, and the no-link article-body rule.
...
assert 'datePublished":"2026-08-05"' in article
assert '<title>Superbuy First Parcel Workflow 2026:' in article
assert '@media(max-width:640px)' in article
body_match = re.search(r'<article class="article">([\s\S]*?)</article>', article)
assert body_match, "article body missing"
body = body_match.group(1)
assert not re.findall(r'href="([^"]+)"', body), "article body must not contain links"
plain = re.sub(r'<[^>]+>', ' ', body)
...
# Validate every internal link used by the article page.
for href in re.findall(r'href="(/[^"]*)"', article):
    if href == "/":
        target = Path("superbuyvip.shop/index.html")
    elif href.endswith("/"):
        target = Path("superbuyvip.shop") / href.lstrip("/") / "index.html"
    else:
        continue
    assert target.exists(), f"broken internal link: {href}"
